...
-- command
----------------------------------
RSGCore.Commands.Add('pet_find', locale('sv_command_find'), {}, false, function(source)
    TriggerClientEvent('rsg-companions:client:getcompanionlocation', source)
end)

RSGCore.Commands.Add('pet_menu', locale('sv_command_menu'), {}, false, function(source)
    TriggerClientEvent('rsg-companions:client:mypetsactions', source) -- main menu
end)

RSGCore.Commands.Add('pet_stats', locale('sv_command_status'), {}, false, function(source)
    TriggerClientEvent('rsg-companions:client:mypets', source) -- menu stats
end)

RSGCore.Commands.Add('pet_games', locale('sv_command_games'), {}, false, function(source)
...
RegisterServerEvent('rsg-companions:server:SetCompanionsUnActive', function(id, stableid)
    ...
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    local activecompanion = MySQL.scalar.await('SELECT id FROM player_companions WHERE citizenid = ? AND active = ?', {Player.PlayerData.citizenid, false})
    MySQL.update('UPDATE player_companions SET active = ? WHERE id = ? AND citizenid = ?', { false, activecompanion, Player.PlayerData.citizenid })
    MySQL.update('UPDATE player_companions SET active = ? WHERE id = ? AND citizenid = ?', { false, id, Player.PlayerData.citizenid })
    MySQL.update('UPDATE player_companions SET stable = ? WHERE id = ? AND citizenid = ?', { stableid, id, Player.PlayerData.citizenid })
end)
...
RSGCore.Commands.Add('petsrevive', locale('sv_command_revive'), {}, false, function(source)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    local success, result = pcall(MySQL.query.await, 'SELECT * FROM player_companions WHERE citizenid = @citizenid AND active = @active', { ['@citizenid'] = Player.PlayerData.citizenid, ['@active'] = 1 })
...
lib.cron.new(Config.AnimalCronJob, function ()
    ...
    for i = 1, #result do
        ...
        local companionname = result[i].name
        local ownercid = result[i].citizenid
        local companionData = json.decode(result[i].companiondata)

        if not companionData or
            type(companionData.hunger) ~= "number" or
            type(companionData.thirst) ~= "number" or
            type(companionData.happiness) ~= "number" or
            type(companionData.dirt) ~= "number" then
            return
        end

        local id_comp = companionData.companionid
        ...
        if days > Config.CompanionDieAge or (companiontype == 'a_c_dogrufus_01' and days >= Config.StarterCompanionDieAge) then -- live <= 0 or 
            local successB, resultB = pcall(MySQL.update, 'DELETE FROM player_companions WHERE id = ?', {id})
            if not successB then return end
            ...
            local successC, resultC = pcall(MySQL.insert, 'INSERT INTO telegrams (citizenid, recipient, sender, sendername, subject, sentDate, message) VALUES (?, ?, ?, ?, ?, ?, ?)',
            {   ownercid,
                locale('sv_telegram_owner'),
                '22222222',
                locale('sv_telegram_stables'),
                companionname..' '..locale('sv_telegram_away'),
                os.date('%x'),
        ...
        companionData.scale = math.min(1.0, 0.5 + 0.1 * companionData.age)    -- update animal scale
        ...
        if updateNeeded then
            local updatedData = json.encode(companionData)
            local successD, resultD = pcall(MySQL.update, 'UPDATE player_companions SET companiondata = ? WHERE id = ?', {updatedData, id})
            if not successD then return end
            TriggerEvent('rsg-companions:server:updateanimals')
        end
    end
